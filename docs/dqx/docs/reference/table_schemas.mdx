...
Once these tables are populated, see the [Query Results Cookbook](/docs/reference/query_results_cookbook) for ready-to-use SQL and PySpark recipes that trace errors and warnings across runs, rows, and check definitions.
...
| `metric_value`         | STRING               | All values are stored as strings. Numeric metrics are string-encoded integers (cast with `CAST(metric_value AS INT)`). For `check_metrics`, this is a JSON string: `[{"check_name": "...", "rule_fingerprint": "...", "error_count": N, "warning_count": N, "user_metadata": {...}}, ...]`. |
